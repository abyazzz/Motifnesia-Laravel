...
<body class="min-h-screen flex items-center justify-center" style="background: linear-gradient(135deg, #FFF8F0 0%, #FFE4D6 100%);">
  ...
  <div class="w-full max-w-5xl mx-4 bg-white rounded-3xl shadow-2xl overflow-hidden flex animate-fade-in-up">
    ...
    <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden" style="background: linear-gradient(135deg, #8B4513 0%, #D2691E 100%);">
      ...
      <div class="relative z-10 flex flex-col justify-center items-center w-full px-12 py-16 text-white text-center">
        ...
        <p class="text-lg opacity-90 mb-8">Daftar sekarang untuk menikmati semua fitur Motifnesia</p>
        <a href="{{ route('auth.register') }}" 
           onclick="handleTransition(event, '{{ route('auth.register') }}')"
    ...
    <div class="flex-1 flex items-center justify-center px-8 py-12 lg:px-16">
      <div class="w-full max-w-sm">
        ...
        <div>
          <h2 class="text-4xl font-bold mb-2" style="color: #8B4513;">Sign In</h2>
          ...
          <form action="{{ route('auth.doLogin') }}" method="POST" class="space-y-4">
            ...
            <div class="relative">
              ...
              <button type="button" onclick="togglePassword('loginPassword', 'loginEye')" 
                ...
                <svg id="loginEye" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-center justify-between text-sm">
              <label class="flex items-center text-gray-600 cursor-pointer">
                <input type="checkbox" name="remember" class="mr-2 rounded" style="accent-color: #8B4513;" />
                <span>Remember me</span>
              </label>
              <a href="{{ route('auth.forgot') }}" class="hover:underline" style="color: #D2691E;">Forgot password?</a>
            </div>
          ...
          <div class="lg:hidden mt-6 text-center">
            <p class="text-gray-600 text-sm">Belum punya akun? 
              <a href="{{ route('auth.register') }}" class="font-bold" style="color: #8B4513;" onclick="handleTransition(event, '{{ route('auth.register') }}')">Daftar</a>
            </p>
          </div>
        </div>
      </div>
